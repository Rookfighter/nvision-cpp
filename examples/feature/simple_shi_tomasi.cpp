...
int main(int argc, const char **argv)
{
    if(argc != 3)
    {
        std::cerr << "usage: simple_shi_tomasi <in-file> <out-file>" << std::endl;
        return -1;
    }
    ...
    // representation.
    nvision::ShiTomasiFeature<nvision::float32> detector;

    // Extract feature points from the image.
    using FeatureMatrix = typename nvision::ShiTomasiFeature<nvision::float32>::FeatureMatrix;
