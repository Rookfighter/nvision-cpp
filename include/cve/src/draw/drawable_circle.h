...
#ifndef CVE_DRAWABLE_CIRCLE_H_
#define CVE_DRAWABLE_CIRCLE_H_
        ...
        namespace internal
        {
            ...
            void drawOutlined(Image<ColorSpace> &img, const Circle &circle, const Color<ColorSpace> &color)
            {
                ...
                while(px >= py)
                {
                    ...
                    if(x >= 0 && x < img.cols() &&
                       y >= 0 && y < img.rows())
                    {
                        img(y, x) = cve::color::toPixel(color);
                    }
            ...
            void drawFilled(Image<ColorSpace> &img, const Circle &circle, const Color<ColorSpace> &color)
            {
                ...
                for(Index c = topLeft.x(); c < botRight.x(); ++c)
                {
                    ...
                    {
                        const auto dx = static_cast<cve::float32>(c) - circle.center.x();
                        const auto dy = static_cast<cve::float32>(r) - circle.center.y();
